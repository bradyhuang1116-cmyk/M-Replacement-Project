...
y_files = sorted(f for f in TIF_DIR.glob('*216*')
                 if f.suffix.lower() in ('.tif', '.tiff', '.pdf', '.png', '.jpg'))
print(f"找到 {len(y_files)} 个测试文件 (仅216)\n")
...
def locate_prefix_by_ocr_with_bounds(image, enlarged_image, code_re):
    ...
    """
    # ── 连通域分析（在放大图上做轻度闭运算，连通断裂笔画但不膨胀） ──
    if len(enlarged_image.shape) == 3:
        gray_cc = cv2.cvtColor(enlarged_image, cv2.COLOR_RGB2GRAY)
    else:
        gray_cc = enlarged_image.copy()
    ...
    img_h_cc, img_w_cc = enlarged_image.shape[:2]
    cc_boxes = []
    for i in range(1, num_labels):
        sx, sy, sw, sh, area = stats_cc[i]
        if area < 50 or sw < 5 or sh < 5:
            continue
...
def diagnose_purple(f):
    ...
    for idx, (code_p1, rx, ry, rw, rh, est_code_x) in enumerate(rough_candidates):
        ...
        if result:
            ...
            # 绘制bound框可视化图
            vis_bound = enlarged.copy()
            # 红色：原始bbox
            cv2.rectangle(vis_bound, (rawx, rawy), (rawx+raww, rawy+rawh), (0, 0, 255), 3)
            # 绿色：约束后bbox
            cv2.rectangle(vis_bound, (cx, cy), (cx+cw, cy+ch), (0, 255, 0), 3)
            # 绘制CC框
            if cc_info:
                for (bx, by, bw, bh) in cc_info['line_ccs']:
                    cv2.rectangle(vis_bound, (bx, by), (bx+bw, by+bh), (255, 200, 0), 2)
            save_img(f'{prefix}_x16_bound_{idx}.jpg', vis_bound)
