#!/usr/bin/env python3
"""批量测试绿框检测 v2（三层递进 + 单元格定位），server 模式。"""
...
OUT_DIR = os.path.join("diagnostic_output", "green_batch_v2")
...
EXPECTED = {
    ...
    "YA147C070P3335_0-脱敏.tif":  "YA147C070P3335",
...
def run_one(fname):
    ...
    # 绿框搜索区: 右下 3/8 宽 × 1/6 高（同 detect_all_regions）
    gx = int(w * 5 / 8)
    gy = int(h * 5 / 6)
    ...
    sh, sw = sub_scaled.shape[:2]

    # 保存搜索区原图
    save_img(os.path.join(out_dir, "01_green_sub.jpg"), cv2.cvtColor(sub_scaled, cv2.COLOR_RGB2BGR))
    ...
    if cell:
        cv2.rectangle(vis, (cell.x, cell.y), (cell.x2, cell.y2), (0, 255, 0), 2)
    save_img(os.path.join(out_dir, "02_cell_detected.jpg"), vis)
    ...
    if result:
        y_text, bbox = result
        # 绘制结果
        vis2 = cv2.cvtColor(sub_scaled.copy(), cv2.COLOR_RGB2BGR)
        cv2.rectangle(vis2, (bbox.x, bbox.y), (bbox.x2, bbox.y2), (0, 0, 255), 2)
        cv2.putText(vis2, y_text, (bbox.x, bbox.y - 5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        if cell:
            cv2.rectangle(vis2, (cell.x, cell.y), (cell.x2, cell.y2), (0, 255, 0), 1)
        save_img(os.path.join(out_dir, "03_result.jpg"), vis2)
...
def main():
    ...
    summary = f"Summary (server v2): {ok_count}/{len(files)} OK"
    print(f"\n{summary}")
