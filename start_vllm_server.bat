...
set CONFIG_PATH=%~dp0vllm_config.json
...
docker run -d ^
    ...
    -v "%CONFIG_PATH%:/config/vllm_config.json:ro" ^
    %IMAGE% ^
    bash -c "mkdir -p /home/paddleocr/.paddlex/official_models && cp -r /models/PaddleOCR-VL-1.5 /home/paddleocr/.paddlex/official_models/PaddleOCR-VL-1.5 && paddleocr genai_server --model_name PaddleOCR-VL-1.5-0.9B --host 0.0.0.0 --port 8080 --backend vllm --backend_config /config/vllm_config.json"
